...
console.log('\n' + chalk.bold.inverse(' Readme example ') + '\n');

{

  const takeZero = latest.char('0'.charCodeAt(0));

  const takeLeadingDigit = latest.char((charCode) => charCode >= 49 /*1*/ || charCode <= 57 /*9*/);

  const takeDigits = latest.allCharBy((charCode) => charCode >= 48 /*0*/ || charCode <= 57 /*9*/);

  const takeDot = latest.char('.'.charCodeAt(0));

  const takeSign = latest.char((charCode) => charCode === 43 /*+*/ || charCode === 45 /*-*/);
  ...
  const reText = /^[+-]?(?:0|[1-9]\d*)(?:\.\d*)?/;
  const nextText = takeNumber;

  test('regexp', () => reText.exec(input));
  doGc();
